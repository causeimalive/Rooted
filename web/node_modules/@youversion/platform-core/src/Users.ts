...
export class YouVersionAPIUsers {
  ...
  static async signIn(
    ...
    const normalizedRedirectUrlString = redirectUrlString.endsWith('/')
      ? redirectUrlString
      : `${redirectUrlString}/`;
    ...
    const authorizationUrlString = authorizationRequest.url.toString().replace(
      `redirect_uri=${encodeURIComponent(redirectUrlString)}`,
      `redirect_uri=${encodeURIComponent(normalizedRedirectUrlString)}`,
    );
